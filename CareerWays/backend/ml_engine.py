...
class NLPEngine:
    ...
    def extract_skills(self, text):
        ...
        filtered_skills = [
            skill for skill in identified_skills if skill not in excluded_terms]
                    ...
                    if keyword in sentence_lower:
                        ...
                        for i, word in enumerate(words):
                            if keyword in word:
                                ...
                                ) + phrase[1:] if phrase else phrase
...
class RecommendationEngine:
    ...
    def _prepare_embeddings(self):
        ...
        try:
            self.course_embeddings = self.vectorizer.fit_transform(
                course_texts)
        except:
            self.course_embeddings = None
    ...
    def calculate_match_score(user_skills, user_interests, course_skills, course_keywords):
        ...
        if user_skills and course_skills:
            ...
            skill_match = min(1.0, skill_match +
                              partial_matches / max(len(course_skills), 1))

        # Enhanced interest matching
        if user_interests and course_keywords:
            user_interests_set = set([interest.lower()
            ...
            interest_match = len(matching_interests) / \
                max(len(course_keywords), 1)
        ...
        base_score = (skill_match * 0.5 + interest_match *
                      0.3 + semantic_bonus * 0.2) * 100
